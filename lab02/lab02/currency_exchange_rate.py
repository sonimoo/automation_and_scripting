...
def setup_logging(project_root):
    """Настроить логирование: консоль + файл error.log в корне проекта."""
    log_path = os.path.join(project_root, "error.log")
    logger = logging.getLogger("currency_exchange_rate")
    logger.setLevel(logging.INFO)

    # Убрать дубликаты хэндлеров при повторном импорте
    if logger.handlers:
        return logger
...
def validate_date(date_str):
    """Проверить формат даты и границы."""
    try:
...
def save_json(project_root, from_cur, to_cur, date_str, payload):
    """Сохранить ответ сервиса в data/PROJECT_ROOT."""
    data_dir = os.path.join(project_root, "data")
    os.makedirs(data_dir, exist_ok=True)
    safe_from = from_cur.upper()
    safe_to = to_cur.upper()
    filename = f"{safe_from}_to_{safe_to}_{date_str}.json"
...
def call_service(from_cur, to_cur, date_str, api_key, timeout=8):
    ...
    Формат: POST на SERVICE_URL?from=...&to=...&date=... с form-data key=APIKEY
    """
...
def main():
    ...
    parser.add_argument("to_currency", help="Currency to convert to (e.g., EUR)")
    parser.add_argument("date", help="Date YYYY-MM-DD")
    parser.add_argument("--key", help="API key (overrides env)", default=None)
    ...
    # Валюты: предупреждаем, но не обязательно строго требуем
    ...
    api_key = args.key or os.environ.get("API_KEY") or DEFAULT_API_KEY
    ...
    try:
        ...
        print("Ошибка: неверный ответ от сервиса (не JSON).")
        sys.exit(2)

    # Если сервис вернул поле error
    ...
    # Ожидаем, что ответ содержит какой-то data/rate
    if not resp_json:
